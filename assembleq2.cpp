...
void structure_full(CsrMatrixCpu<double>& matrix, std::vector<FullTriangleQ2>& elements)
{
    ...
    size_t* num_nonzeros = new size_t[matrix._numrows];
    for (size_t i{0}; i < matrix._numrows; ++i)
        num_nonzeros[i] = 0;

    size_t* colind = new size_t[max_rowlength*matrix._numrows];
    ...
    for (size_t i{0}; i < elements.size(); ++i)
    {
        ...
        for (size_t node1{0}; node1 < 6; ++node1)
        {
            for (size_t node2{0}; node2 < 6; ++node2)
            {
                ...
                if (a >= 0 && static_cast<size_t>(a) < matrix._numrows)
                {
                    size_t j{0};
    ...
    matrix._rowptr[matrix._numrows] = num_values;
...
void assemble_id(CsrMatrixCpu<double>& matrix, std::vector<Node>& nodes, std::vector<TriangleQ2>& elements)
{
    for (const auto& elem : elements)
    {
        ...
        matrix.add(elem.nodeA, elem.nodeA, ( 3.0*aacc + 6.0*abcd + 3.0*bbdd ) / detB / 6.0);
        matrix.add(elem.nodeA, elem.nodeB, (     aacc +     abcd            ) / detB / 6.0);
        matrix.add(elem.nodeA, elem.nodeC, (                abcd +     bbdd ) / detB / 6.0);
        matrix.add(elem.nodeA, elem.nodeD, (-4.0*aacc - 4.0*abcd            ) / detB / 6.0);
        //trix.add(elem.nodeA, elem.nodeE, (                                ) / detB / 6.0);
        matrix.add(elem.nodeA, elem.nodeF, (          - 4.0*abcd - 4.0*bbdd ) / detB / 6.0);

        matrix.add(elem.nodeB, elem.nodeA, (     aacc +     abcd            ) / detB / 6.0);
        matrix.add(elem.nodeB, elem.nodeB, ( 3.0*aacc                       ) / detB / 6.0);
        matrix.add(elem.nodeB, elem.nodeC, (          -     abcd            ) / detB / 6.0);
        matrix.add(elem.nodeB, elem.nodeD, (-4.0*aacc - 4.0*abcd            ) / detB / 6.0);
        matrix.add(elem.nodeB, elem.nodeE, (            4.0*abcd            ) / detB / 6.0);
        //trix.add(elem.nodeB, elem.nodeF, (                                ) / detB / 6.0);

        matrix.add(elem.nodeC, elem.nodeA, (                abcd +     bbdd ) / detB / 6.0);
        matrix.add(elem.nodeC, elem.nodeB, (          -     abcd            ) / detB / 6.0);
        matrix.add(elem.nodeC, elem.nodeC, (                       3.0*bbdd ) / detB / 6.0);
        //trix.add(elem.nodeC, elem.nodeD, (                                ) / detB / 6.0);
        matrix.add(elem.nodeC, elem.nodeE, (            4.0*abcd            ) / detB / 6.0);
        matrix.add(elem.nodeC, elem.nodeF, (          - 4.0*abcd - 4.0*bbdd ) / detB / 6.0);

        matrix.add(elem.nodeD, elem.nodeA, (-4.0*aacc - 4.0*abcd            ) / detB / 6.0);
        matrix.add(elem.nodeD, elem.nodeB, (-4.0*aacc - 4.0*abcd            ) / detB / 6.0);
        //trix.add(elem.nodeD, elem.nodeC, (                                ) / detB / 6.0);
        matrix.add(elem.nodeD, elem.nodeD, ( 8.0*aacc + 8.0*abcd + 8.0*bbdd ) / detB / 6.0);
        matrix.add(elem.nodeD, elem.nodeE, (          - 8.0*abcd - 8.0*bbdd ) / detB / 6.0);
        matrix.add(elem.nodeD, elem.nodeF, (            8.0*abcd            ) / detB / 6.0);

        //trix.add(elem.nodeE, elem.nodeA, (                                ) / detB / 6.0);
        matrix.add(elem.nodeE, elem.nodeB, (            4.0*abcd            ) / detB / 6.0);
        matrix.add(elem.nodeE, elem.nodeC, (            4.0*abcd            ) / detB / 6.0);
        matrix.add(elem.nodeE, elem.nodeD, (          - 8.0*abcd - 8.0*bbdd ) / detB / 6.0);
        matrix.add(elem.nodeE, elem.nodeE, ( 8.0*aacc + 8.0*abcd + 8.0*bbdd ) / detB / 6.0);
        matrix.add(elem.nodeE, elem.nodeF, (-8.0*aacc - 8.0*abcd            ) / detB / 6.0);

        matrix.add(elem.nodeF, elem.nodeA, (          - 4.0*abcd - 4.0*bbdd ) / detB / 6.0);
        //trix.add(elem.nodeF, elem.nodeB, (                                ) / detB / 6.0);
        matrix.add(elem.nodeF, elem.nodeC, (          - 4.0*abcd - 4.0*bbdd ) / detB / 6.0);
        matrix.add(elem.nodeF, elem.nodeD, (            8.0*abcd            ) / detB / 6.0);
        matrix.add(elem.nodeF, elem.nodeE, (-8.0*aacc - 8.0*abcd            ) / detB / 6.0);
        matrix.add(elem.nodeF, elem.nodeF, ( 8.0*aacc + 8.0*abcd + 8.0*bbdd ) / detB / 6.0);

        //matrix.add(elem.nodeA, elem.nodeA, ( 3.0*A*A + 6.0*A*B + 3.0*B*B
        //                                   + 3.0*C*C + 6.0*C*D + 3.0*D*D ) * detB / 6.0);
        //matrix.add(elem.nodeA, elem.nodeA, ( A*A + 2.0*A*B + B*B
        //                                   + C*C + 2.0*C*D + D*D ) * detB / 2.0);
        //matrix.add(elem.nodeA, elem.nodeA, ( (A+B)*(A+B)
        //                                   + (C+D)*(C+D) ) * detB / 2.0);
/*
        const double A{ d/detB};
        const double B{-c/detB};
        ...
        matrix.add(elem.nodeA, elem.nodeA, ( (-    A-    B)*(-    A-    B) + (-    C-    D)*(-    C-    D)
                                           + (-    A-    B)*(-    A-    B) + (-    C-    D)*(-    C-    D)
                                           + (     A+    B)*(     A+    B) + (     C+    D)*(     C+    D) ) * detB / 6.0);
        matrix.add(elem.nodeA, elem.nodeB, ( (-    A-    B)*(-    A      ) + (-    C-    D)*(-    C      )
                                           + (-    A-    B)*(     A      ) + (-    C-    D)*(     C      )
                                           + (     A+    B)*(     A      ) + (     C+    D)*(     C      ) ) * detB / 6.0);
        matrix.add(elem.nodeA, elem.nodeC, ( (-    A-    B)*(           B) + (-    C-    D)*(           D)
                                           + (-    A-    B)*(      -    B) + (-    C-    D)*(      -    D)
                                           + (     A+    B)*(           B) + (     C+    D)*(           D) ) * detB / 6.0);
        matrix.add(elem.nodeA, elem.nodeD, ( (-    A-    B)*( 2.0*A      ) + (-    C-    D)*( 2.0*C      )
                                           + (-    A-    B)*(      -2.0*B) + (-    C-    D)*(      -2.0*D)
                                           + (     A+    B)*(-2.0*A-2.0*B) + (     C+    D)*(-2.0*C-2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeA, elem.nodeE, ( (-    A-    B)*( 2.0*A      ) + (-    C-    D)*( 2.0*C      )
                                           + (-    A-    B)*(       2.0*B) + (-    C-    D)*(       2.0*D)
                                           + (     A+    B)*( 2.0*A+2.0*B) + (     C+    D)*( 2.0*C+2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeA, elem.nodeF, ( (-    A-    B)*(-2.0*A      ) + (-    C-    D)*(-2.0*C      )
                                           + (-    A-    B)*(       2.0*B) + (-    C-    D)*(       2.0*D)
                                           + (     A+    B)*(-2.0*A-2.0*B) + (     C+    D)*(-2.0*C-2.0*D) ) * detB / 6.0);

        matrix.add(elem.nodeB, elem.nodeA, ( (-    A      )*(-    A-    B) + (-    C      )*(-    C-    D)
                                           + (     A      )*(-    A-    B) + (     C      )*(-    C-    D)
                                           + (     A      )*(     A+    B) + (     C      )*(     C+    D) ) * detB / 6.0);
        matrix.add(elem.nodeB, elem.nodeB, ( (-    A      )*(-    A      ) + (-    C      )*(-    C      )
                                           + (     A      )*(     A      ) + (     C      )*(     C      )
                                           + (     A      )*(     A      ) + (     C      )*(     C      ) ) * detB / 6.0);
        matrix.add(elem.nodeB, elem.nodeC, ( (-    A      )*(           B) + (-    C      )*(           D)
                                           + (     A      )*(      -    B) + (     C      )*(      -    D)
                                           + (     A      )*(           B) + (     C      )*(           D) ) * detB / 6.0);
        matrix.add(elem.nodeB, elem.nodeD, ( (-    A      )*( 2.0*A      ) + (-    C      )*( 2.0*C      )
                                           + (     A      )*(      -2.0*B) + (     C      )*(      -2.0*D)
                                           + (     A      )*(-2.0*A-2.0*B) + (     C      )*(-2.0*C-2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeB, elem.nodeE, ( (-    A      )*( 2.0*A      ) + (-    C      )*( 2.0*C      )
                                           + (     A      )*(       2.0*B) + (     C      )*(       2.0*D)
                                           + (     A      )*( 2.0*A+2.0*B) + (     C      )*( 2.0*C+2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeB, elem.nodeF, ( (-    A      )*(-2.0*A      ) + (-    C      )*(-2.0*C      )
                                           + (     A      )*(       2.0*B) + (     C      )*(       2.0*D)
                                           + (     A      )*(-2.0*A-2.0*B) + (     C      )*(-2.0*C-2.0*D) ) * detB / 6.0);

        matrix.add(elem.nodeC, elem.nodeA, ( (           B)*(-    A-    B) + (           D)*(-    C-    D)
                                           + (      -    B)*(-    A-    B) + (      -    D)*(-    C-    D)
                                           + (           B)*(     A+    B) + (           D)*(     C+    D) ) * detB / 6.0);
        matrix.add(elem.nodeC, elem.nodeB, ( (           B)*(-    A      ) + (           D)*(-    C      )
                                           + (      -    B)*(     A      ) + (      -    D)*(     C      )
                                           + (           B)*(     A      ) + (           D)*(     C      ) ) * detB / 6.0);
        matrix.add(elem.nodeC, elem.nodeC, ( (           B)*(           B) + (           D)*(           D)
                                           + (      -    B)*(      -    B) + (      -    D)*(      -    D)
                                           + (           B)*(           B) + (           D)*(           D) ) * detB / 6.0);
        matrix.add(elem.nodeC, elem.nodeD, ( (           B)*( 2.0*A      ) + (           D)*( 2.0*C      )
                                           + (      -    B)*(      -2.0*B) + (      -    D)*(      -2.0*D)
                                           + (           B)*(-2.0*A-2.0*B) + (           D)*(-2.0*C-2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeC, elem.nodeE, ( (           B)*( 2.0*A      ) + (           D)*( 2.0*C      )
                                           + (      -    B)*(       2.0*B) + (      -    D)*(       2.0*D)
                                           + (           B)*( 2.0*A+2.0*B) + (           D)*( 2.0*C+2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeC, elem.nodeF, ( (           B)*(-2.0*A      ) + (           D)*(-2.0*C      )
                                           + (      -    B)*(       2.0*B) + (      -    D)*(       2.0*D)
                                           + (           B)*(-2.0*A-2.0*B) + (           D)*(-2.0*C-2.0*D) ) * detB / 6.0);

        matrix.add(elem.nodeD, elem.nodeA, ( ( 2.0*A      )*(-    A-    B) + ( 2.0*C      )*(-    C-    D)
                                           + (      -2.0*B)*(-    A-    B) + (      -2.0*D)*(-    C-    D)
                                           + (-2.0*A-2.0*B)*(     A+    B) + (-2.0*C-2.0*D)*(     C+    D) ) * detB / 6.0);
        matrix.add(elem.nodeD, elem.nodeB, ( ( 2.0*A      )*(-    A      ) + ( 2.0*C      )*(-    C      )
                                           + (      -2.0*B)*(     A      ) + (      -2.0*D)*(     C      )
                                           + (-2.0*A-2.0*B)*(     A      ) + (-2.0*C-2.0*D)*(     C      ) ) * detB / 6.0);
        matrix.add(elem.nodeD, elem.nodeC, ( ( 2.0*A      )*(           B) + ( 2.0*C      )*(           D)
                                           + (      -2.0*B)*(      -    B) + (      -2.0*D)*(      -    D)
                                           + (-2.0*A-2.0*B)*(           B) + (-2.0*C-2.0*D)*(           D) ) * detB / 6.0);
        matrix.add(elem.nodeD, elem.nodeD, ( ( 2.0*A      )*( 2.0*A      ) + ( 2.0*C      )*( 2.0*C      )
                                           + (      -2.0*B)*(      -2.0*B) + (      -2.0*D)*(      -2.0*D)
                                           + (-2.0*A-2.0*B)*(-2.0*A-2.0*B) + (-2.0*C-2.0*D)*(-2.0*C-2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeD, elem.nodeE, ( ( 2.0*A      )*( 2.0*A      ) + ( 2.0*C      )*( 2.0*C      )
                                           + (      -2.0*B)*(       2.0*B) + (      -2.0*D)*(       2.0*D)
                                           + (-2.0*A-2.0*B)*( 2.0*A+2.0*B) + (-2.0*C-2.0*D)*( 2.0*C+2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeD, elem.nodeF, ( ( 2.0*A      )*(-2.0*A      ) + ( 2.0*C      )*(-2.0*C      )
                                           + (      -2.0*B)*(       2.0*B) + (      -2.0*D)*(       2.0*D)
                                           + (-2.0*A-2.0*B)*(-2.0*A-2.0*B) + (-2.0*C-2.0*D)*(-2.0*C-2.0*D) ) * detB / 6.0);

        matrix.add(elem.nodeE, elem.nodeA, ( ( 2.0*A      )*(-    A-    B) + ( 2.0*C      )*(-    C-    D)
                                           + (       2.0*B)*(-    A-    B) + (       2.0*D)*(-    C-    D)
                                           + ( 2.0*A+2.0*B)*(     A+    B) + ( 2.0*C+2.0*D)*(     C+    D) ) * detB / 6.0);
        matrix.add(elem.nodeE, elem.nodeB, ( ( 2.0*A      )*(-    A      ) + ( 2.0*C      )*(-    C      )
                                           + (       2.0*B)*(     A      ) + (       2.0*D)*(     C      )
                                           + ( 2.0*A+2.0*B)*(     A      ) + ( 2.0*C+2.0*D)*(     C      ) ) * detB / 6.0);
        matrix.add(elem.nodeE, elem.nodeC, ( ( 2.0*A      )*(           B) + ( 2.0*C      )*(           D)
                                           + (       2.0*B)*(      -    B) + (       2.0*D)*(      -    D)
                                           + ( 2.0*A+2.0*B)*(           B) + ( 2.0*C+2.0*D)*(           D) ) * detB / 6.0);
        matrix.add(elem.nodeE, elem.nodeD, ( ( 2.0*A      )*( 2.0*A      ) + ( 2.0*C      )*( 2.0*C      )
                                           + (       2.0*B)*(      -2.0*B) + (       2.0*D)*(      -2.0*D)
                                           + ( 2.0*A+2.0*B)*(-2.0*A-2.0*B) + ( 2.0*C+2.0*D)*(-2.0*C-2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeE, elem.nodeE, ( ( 2.0*A      )*( 2.0*A      ) + ( 2.0*C      )*( 2.0*C      )
                                           + (       2.0*B)*(       2.0*B) + (       2.0*D)*(       2.0*D)
                                           + ( 2.0*A+2.0*B)*( 2.0*A+2.0*B) + ( 2.0*C+2.0*D)*( 2.0*C+2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeE, elem.nodeF, ( ( 2.0*A      )*(-2.0*A      ) + ( 2.0*C      )*(-2.0*C      )
                                           + (       2.0*B)*(       2.0*B) + (       2.0*D)*(       2.0*D)
                                           + ( 2.0*A+2.0*B)*(-2.0*A-2.0*B) + ( 2.0*C+2.0*D)*(-2.0*C-2.0*D) ) * detB / 6.0);

        matrix.add(elem.nodeF, elem.nodeA, ( (-2.0*A      )*(-    A-    B) + (-2.0*C      )*(-    C-    D)
                                           + (       2.0*B)*(-    A-    B) + (       2.0*D)*(-    C-    D)
                                           + (-2.0*A-2.0*B)*(     A+    B) + (-2.0*C-2.0*D)*(     C+    D) ) * detB / 6.0);
        matrix.add(elem.nodeF, elem.nodeB, ( (-2.0*A      )*(-    A      ) + (-2.0*C      )*(-    C      )
                                           + (       2.0*B)*(     A      ) + (       2.0*D)*(     C      )
                                           + (-2.0*A-2.0*B)*(     A      ) + (-2.0*C-2.0*D)*(     C      ) ) * detB / 6.0);
        matrix.add(elem.nodeF, elem.nodeC, ( (-2.0*A      )*(           B) + (-2.0*C      )*(           D)
                                           + (       2.0*B)*(      -    B) + (       2.0*D)*(      -    D)
                                           + (-2.0*A-2.0*B)*(           B) + (-2.0*C-2.0*D)*(           D) ) * detB / 6.0);
        matrix.add(elem.nodeF, elem.nodeD, ( (-2.0*A      )*( 2.0*A      ) + (-2.0*C      )*( 2.0*C      )
                                           + (       2.0*B)*(      -2.0*B) + (       2.0*D)*(      -2.0*D)
                                           + (-2.0*A-2.0*B)*(-2.0*A-2.0*B) + (-2.0*C-2.0*D)*(-2.0*C-2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeF, elem.nodeE, ( (-2.0*A      )*( 2.0*A      ) + (-2.0*C      )*( 2.0*C      )
                                           + (       2.0*B)*(       2.0*B) + (       2.0*D)*(       2.0*D)
                                           + (-2.0*A-2.0*B)*( 2.0*A+2.0*B) + (-2.0*C-2.0*D)*( 2.0*C+2.0*D) ) * detB / 6.0);
        matrix.add(elem.nodeF, elem.nodeF, ( (-2.0*A      )*(-2.0*A      ) + (-2.0*C      )*(-2.0*C      )
                                           + (       2.0*B)*(       2.0*B) + (       2.0*D)*(       2.0*D)
                                           + (-2.0*A-2.0*B)*(-2.0*A-2.0*B) + (-2.0*C-2.0*D)*(-2.0*C-2.0*D) ) * detB / 6.0);
*/
    }
